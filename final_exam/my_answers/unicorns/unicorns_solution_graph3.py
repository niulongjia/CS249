...
"""
#Startups_and_Investors = pd.read_csv("https://github.com/notpeter/crunchbase-data/blob/master/investments.csv?raw=true")
#Startups_full_list = pd.read_csv("https://github.com/notpeter/crunchbase-data/blob/master/companies.csv?raw=true")
Startups_and_Investors = pd.read_csv('investments.csv')
Startups_full_list = pd.read_csv("companies.csv")
...
def isConnected(node1,node2,Startups_with_a_common_Investor):
    return any((Startups_with_a_common_Investor.company_name_x==node1)&(Startups_with_a_common_Investor.company_name_y==node2))
"""
...
Startups_with_a_common_Investor = pd.merge( left=Startups_and_Investors, right=Startups_and_Investors, how="inner",
                                           left_on="investor_name", right_on="investor_name")[['company_name_x', 'company_name_y']].drop_duplicates()

Startups_with_a_common_Investor = Startups_with_a_common_Investor[
                             Startups_with_a_common_Investor['company_name_x'] != Startups_with_a_common_Investor['company_name_y'] ]
...
for x in Startups_with_a_common_Investor[['company_name_x', 'company_name_y']].values:
    startup0  = fixup(str(x[0]))
    startup1 = fixup(str(x[1]))
...
print "number of connected components:"+str(len(Startups_with_a_common_Investor['company_name_x'].drop_duplicates()))
...
Startups_full_list=Startups_full_list["name"].tolist()
