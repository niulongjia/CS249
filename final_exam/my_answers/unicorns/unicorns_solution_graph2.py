...
"""
Startups_and_Investors_Now = pd.read_csv('investors_in_unicorns.tsv', sep='\t')
Startups_and_Investors_Future = pd.read_csv('investors_in_future_unicorns.tsv', sep='\t')
Startups_and_Investors = pd.concat([Startups_and_Investors_Now, Startups_and_Investors_Future], axis=0)

Startups_now_list = pd.read_csv("unicorns.tsv",sep='\t')
Startups_future_list = pd.read_csv('future_unicorns.tsv',sep='\t')
Startups_full_list = pd.concat([Startups_now_list, Startups_future_list], axis=0)
...
list_of_degree_values = list_of_degree_values[0:150]
...
Startups_future_disconnected=[]
for i in range(0,len(Startups_future_list)):
    Startup_future = Startups_future_list.iloc[i]["Company"]
    sign=0
    for j in range(0,len(Startups_with_a_common_Investor)):
        if (Startups_with_a_common_Investor.iloc[j]["Company_x"]==Startup_future) :
            sign=1
            break
    if (sign==0): 
        Startups_future_disconnected.append(Startup_future)

for i in range(0,len(Startups_future_disconnected)):
    print Startups_future_disconnected[i]